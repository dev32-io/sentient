...
describe("hermesMessageToMirrorEntry", () => {
  it("preserves the real ts (Unix seconds -> ms) for a user message", () => {
    const entry = hermesMessageToMirrorEntry(raw({ role: "user", ts: 1_717_000_000, content: "hi" }));
    expect(entry).toEqual({ kind: "user", ts: 1_717_000_000_000, channel: "text", content: "hi" });
  });

  it("passes through ts already in ms (above the seconds/ms threshold)", () => {
    const entry = hermesMessageToMirrorEntry(raw({ role: "assistant", ts: 1_717_000_000_000, content: "yo" }));
  ...
  it.each([
    ...
    const entry = hermesMessageToMirrorEntry(raw({ role: "user", ts: badTs as unknown as number, content: "hi" }));
  ...
  it("drops tool / tool_call / tool_result / system roles", () => {
    for (const role of ["tool", "tool_call", "tool_result", "system"] as const) {
      expect(hermesMessageToMirrorEntry(raw({ role }))).toBeNull();
    }
  });
});
