...
function fakeAdapter(overrides: Partial<HomeAdapter> = {}): HomeAdapter & { writes: string[] } {
  ...
  return {
    ...
    createConfig: async (kind, id) => ({ outcome: "succeeded", id, entityId: `${kind}.${id}` }),
    ...
    mutateTodo: async (listId) => ({ outcome: "accepted_unverified", listId }),
    calendarEvents: async () => ({ outcome: "succeeded", events: [] }),
    mutateCalendar: async (calendarId) => ({ outcome: "accepted_unverified", calendarId }),
    ...overrides,
  };
}
...
describe("native Home tools", () => {
  it("exposes compact standard product-group definitions without ha-mcp", () => {
    ...
    expect(definitions.map((value) => value.name)).toEqual([
      ...
      "home_remove_scene",
      "home_get_automation",
      "home_create_automation",
      "home_update_automation",
