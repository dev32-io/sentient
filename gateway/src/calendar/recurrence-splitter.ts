...
function recurrenceFromRule(rule: RRule): Recurrence {
  ...
  if (rule.until !== undefined) fields.push(`UNTIL=${formatRfcUntil(rule.until)}`);
...
function formatRfcUntil(value: UtcInstant): string {
...
function terminalFor(previous: CalendarTime): UtcInstant {
  ...
  return `${previous.date.replaceAll("-", "")}T235959Z` as UtcInstant;
}
function normalizeSuccessorRecurrence(
  ...
  const canonical = canonicalizeRecurrence(value);
...
export function splitRecurrence(
  ...
  const normalized = normalizeSuccessorRecurrence(proposedSuccessorRecurrence);
  ...
  if (prefixLast) {
    prefixRecurrence =
      oldRule.count !== undefined
        ...
        : recurrenceFromRule(boundedRule(oldRule, undefined, terminalFor(prefixLast)));
  }
