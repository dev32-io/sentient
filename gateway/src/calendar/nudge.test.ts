...
const occurrence = (
  ...
  importance: "normal" | "important" | "pinned" = "normal",
): Occurrence => ({
...
function store(events: Occurrence[]): CalendarStore {
  return {
    list: () => ({ ok: true, value: events }),
    get: () => ({ ok: false, error: "not-found" }),
    create: () => ({ ok: false, error: "not-implemented" }),
    update: () => ({ ok: false, error: "not-implemented" }),
    delete: () => ({ ok: false, error: "not-implemented" }),
    close: () => {},
  } as CalendarStore;
}
...
describe("composeCalendarNudge", () => {
  it("omits an empty calendar and caps with an overflow line", () => {
    ...
    const result = composeCalendarNudge(store(events), "adult", "UTC", Date.parse("2026-08-05T12:00:00Z"), {
      maxChars: 80,
      maxLines: 4,
    });
  ...
  it("uses household-local today and hides adults events for a child", () => {
    const events = [
      occurrence("Visible", "2026-08-06T00:30:00.000Z"),
      occurrence("Adults", "2026-08-05T12:00:00.000Z", "adults"),
    ];
  ...
  it("queries one household-local date with maxDays=1 at a UTC date boundary", () => {
    ...
    const result = composeCalendarNudge(
      nudgeQuery(
        [
          persistedEvent("Timed today", {
            kind: "timed",
            instant: "2026-08-06T00:30:00.000Z" as UtcInstant,
            timeZoneId: zone as never,
          }),
          persistedEvent("All day today", { kind: "all-day", date: "2026-08-05" }),
          persistedEvent(
            "Previous day",
            { kind: "timed", instant: "2026-08-04T12:00:00.000Z" as UtcInstant, timeZoneId: zone as never },
            "important",
          ),
    ...
    const result = composeCalendarNudge(
      nudgeQuery(
        [
          ...
          persistedEvent("DST timed today", {
            kind: "timed",
            instant: "2026-03-08T07:30:00.000Z" as UtcInstant,
            timeZoneId: zone as never,
          }),
          ...
          persistedEvent("DST recurring", { kind: "all-day", date: "2026-03-07" }, "normal", {
            ...
            exclusions: [
              { kind: "all-day", date: "2026-03-07" },
              { kind: "all-day", date: "2026-03-09" },
            ],
          }),
          persistedEvent("DST previous day", { kind: "all-day", date: "2026-03-07" }, "important"),
          persistedEvent(
            "DST following day",
            { kind: "timed", instant: "2026-03-09T12:00:00.000Z" as UtcInstant, timeZoneId: zone as never },
            "important",
          ),
        ],
        zone,
