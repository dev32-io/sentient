...
export function createCalendarEvent(
  ...
  const result = context.persistence.transaction(
    (tx) => {
    ...
    { actorReminderMutation: checked.value.reminder !== undefined },
  );
  if (!result.ok) {
    if (isAborted(context.signal)) return aborted();
...
function reminderWasMutated(command: MutationCommand): boolean {
  return command.operation === "update" && command.changes.reminder !== undefined;
}
...
function updateOccurrence(
  ...
  const result = context.persistence.transaction(
    ...
    {
      actorReminderMutation: reminderWasMutated(command),
      actorPersonalOnly: isActorPersonalReminderMutation(command),
    },
  );
  if (!result.ok) {
    if (isAborted(context.signal)) return aborted();
...
function updateThisAndFollowing(
  ...
  const result = context.persistence.transaction(
    (tx) => {
      ...
      const existingAtSelected = event.exceptions.find(
...
function deleteWholeSeries(
  ...
  const result = context.persistence.transaction(
    (tx) => {
      ...
      return {
        ok: true,
        value: { operation: "delete", appliedTo: "entire_series", eventId: current.value.id } as const,
      };
    },
    {
      actorReminderMutation: reminderWasMutated(command),
