...
describe("calendar V2 contracts", () => {
  test("accepts model-friendly bounded times and rejects CalendarTime objects", () => {
    ...
    for (const value of [
      ...
      { kind: "timed", instant: "2026-08-05T13:00:00.000Z", timeZoneId: "UTC" },
    ]) {
  ...
  test("requires exactly one recurrence bound and full weekday names", () => {
    ...
    expect(calendarRecurrenceInputSchema.safeParse({ frequency: "daily", count: 2, until: "2026-12-31" }).success).toBe(
      false,
    );
    expect(calendarRecurrenceInputSchema.safeParse({ frequency: "weekly", count: 2 }).success).toBe(false);
    expect(
      calendarRecurrenceInputSchema.safeParse({ frequency: "monthly", weekdays: ["monday"], count: 2 }).success,
    ).toBe(false);
    ...
    expect(wireRRuleSchema.safeParse({ freq: "WEEKLY", byDay: ["MO", "WE"], count: 6, extra: true }).success).toBe(
      false,
    );
  });
  ...
  test("defines private/household/all reads and private/household writes", () => {
    expect(
      calendarQueryInputSchema.safeParse({ from: "2026-08-01", to: "2026-08-31", query: "dinner", scope: "all" })
        .success,
    ).toBe(true);
    ...
    expect(
      calendarQueryInputSchema.safeParse({ from: "2026-08-01", to: "2026-08-31", query: "x".repeat(512) }).success,
    ).toBe(true);
    ...
    expect(calendarCreateInputSchema.safeParse({ ...goldenCalendarFixtures.create, scope: undefined }).success).toBe(
      true,
    );
  ...
  test("requires applyTo and restricts occurrence-local changes", () => {
    ...
    expect(
      calendarMutationCommandSchema.safeParse({
        ...target,
        applyTo: "this_occurrence",
        changes: { title: "Moved", description: null, end: null, group: null, tags: [] },
      }).success,
    ).toBe(true);
    expect(calendarMutationCommandSchema.safeParse({ ...target, changes: { title: "Missing scope" } }).success).toBe(
      false,
    );
    expect(
      calendarMutationCommandSchema.safeParse({ ...target, applyTo: "this_occurrence", changes: { recurrence: null } })
        .success,
    ).toBe(false);
    expect(
      calendarMutationCommandSchema.safeParse({
        ...
        changes: { recurrence: { frequency: "weekly", weekdays: ["friday"], count: 3 } },
      }).success,
    ).toBe(true);
    expect(
      calendarMutationCommandSchema.safeParse({
    ...
    expect(calendarMutationCommandSchema.safeParse({ operation: "delete", eventId: "event-example" }).success).toBe(
      false,
    );
    expect(
      calendarMutationCommandSchema.safeParse({ ...target, applyTo: "entire_series", changes: { eventId: "other" } })
        .success,
    ).toBe(false);
  });
  ...
  test("supports rich clear semantics without allowing forbidden fields", () => {
    expect(
      calendarUpdateChangesSchema.safeParse({ description: null, end: null, group: null, recurrence: null, tags: [] })
        .success,
    ).toBe(true);
    expect(
      calendarOccurrenceChangesSchema.safeParse({ description: null, end: null, group: null, tags: [] }).success,
    ).toBe(true);
  ...
  test("pins V2 envelopes, pages, mutations, and stable errors", () => {
    ...
    expect(
      calendarEventSchema.safeParse({
        eventId: "event-example",
        revision: 3,
        ...goldenCalendarFixtures.create,
      }).success,
    ).toBe(true);
    expect(
      calendarResponseSchema.safeParse({ version: 2, requestId: "r1", body: goldenCalendarFixtures.page }).success,
    ).toBe(true);
    expect(
      calendarResponseSchema.safeParse({ version: 2, requestId: "r1", body: goldenCalendarFixtures.occurrence })
        .success,
    ).toBe(true);
    expect(
      calendarRequestSchema.safeParse({
        version: 2,
        requestId: "r1",
        operation: "list",
        body: { from: "2026", to: "2026-12" },
      }).success,
    ).toBe(true);
    expect(
      calendarRequestSchema.safeParse({
        ...
        requestId: "r1",
        operation: "get",
        body: { eventId: "event-example", originalStart: "2026-08-10T09:00-04:00" },
      }).success,
    ).toBe(true);
    expect(
      calendarRequestSchema.safeParse({
    ...
    expect(
      calendarRequestSchema.safeParse({
        ...
        requestId: "r1",
        operation: "update",
        body: { eventId: "event-example", applyTo: "this_occurrence", changes: { title: "x" } },
      }).success,
    ).toBe(true);
    expect(
      calendarRequestSchema.safeParse({
        ...
        requestId: "r1",
        operation: "update",
        body: { id: "event-example", patch: { title: "old" } },
      }).success,
    ).toBe(false);
    expect(
      calendarMutationResultSchema.safeParse({
        operation: "update",
