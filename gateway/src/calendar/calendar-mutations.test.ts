...
describe("calendar mutation boundary", () => {
  ...
  it("updates one occurrence sparsely, preserves identity, and cancels only that slot", () => {
    ...
    expect(updated).toEqual({
      ok: true,
      value: expect.objectContaining({ appliedTo: "this_occurrence", resultingRevision: 2 }),
    });
    const raw = persistence.read(created.value.eventId as CalendarEventId);
    expect(raw).toMatchObject({ ok: true, value: { revision: 2 } });
    if (!raw.ok) return;
    const exception = raw.value.exceptions.find(
      (value) => String(value.occurrence.kind === "timed" && value.occurrence.instant) === original,
    );
    expect(exception).toMatchObject({
      title: "moved occurrence",
      start: { kind: "timed", instant: "2026-01-12T16:00:00.000Z" },
    });
    ...
    const cleared = mutateCalendarEvent(
      mutation({
        operation: "update",
        eventId: created.value.eventId,
        applyTo: "this_occurrence",
    ...
    expect(clearedState).toMatchObject({
      ok: true,
      value: { exceptions: [{ description: null, end: null, group: null, tags: [] }] },
    });

    const cancelled = mutateCalendarEvent(
      mutation({
        operation: "delete",
        eventId: created.value.eventId,
        applyTo: "this_occurrence",
    ...
    const repeated = mutateCalendarEvent(
      mutation({
        operation: "delete",
        eventId: created.value.eventId,
        applyTo: "this_occurrence",
    ...
    expect(after.ok && after.value.map((value) => String(value.originalStart))).toEqual([
  ...
  it("clears optional occurrence fields and removes a conflicting exdate atomically", () => {
    ...
    expect(
      persistence.transaction((tx) => tx.replaceExclusions(created.value.eventId as CalendarEventId, [second])),
    ...
    expect(state).toMatchObject({
      ok: true,
      value: { revision: 2, exceptions: [{ cancelled: true }], exclusions: [] },
    });
    persistence.close();
  });
  ...
  it("re-anchors every successor slot and preserves the selected duration", () => {
    ...
    const oldFutureExclusion = {
      kind: "timed",
      instant: "2026-01-08T14:00:00.000Z",
      timeZoneId: "America/Toronto",
    } as unknown as CalendarTime;
    expect(persistence.transaction((tx) => tx.replaceExclusions(eventId, [oldFutureExclusion]))).toEqual({
      ok: true,
      value: undefined,
    });
    const result = mutateCalendarEvent(
    ...
    if (!result.ok) return;
    const successor = persistence.read(
      (result.value as unknown as { successorEventId: CalendarEventId }).successorEventId,
    );
    expect(successor).toMatchObject({
      ok: true,
      value: {
        start: { instant: "2026-01-06T16:00:00.000Z" },
        end: { instant: "2026-01-06T17:00:00.000Z" },
    ...
    if (!second.ok) return;
    const endResult = mutateCalendarEvent(
      mutation({
        operation: "update",
        eventId: second.value.eventId,
        applyTo: "this_and_following",
        originalStart: "2026-01-06T14:00:00.000Z",
        changes: { end: "2026-01-06T13:00:00-05:00" },
      }),
      endOnly,
      config,
    );
    ...
    if (endResult.ok) {
      const endSuccessor = endOnly.read(
        (endResult.value as unknown as { successorEventId: CalendarEventId }).successorEventId,
      );
      expect(endSuccessor).toMatchObject({
        ok: true,
        value: { start: { instant: "2026-01-06T14:00:00.000Z" }, end: { instant: "2026-01-06T18:00:00.000Z" } },
      });
    }
    endOnly.close();

    const allDay = openCalendarPersistence(cap(root()), config);
    const allDayCreated = createCalendarEvent(
      createInput({
    ...
    const allDayResult = mutateCalendarEvent(
      mutation({
        operation: "update",
        eventId: allDayCreated.value.eventId,
        applyTo: "this_and_following",
        originalStart: "2026-01-06",
        changes: { start: "2026-01-07", end: "2026-01-09" },
      }),
      allDay,
      config,
    );
    ...
    if (allDayResult.ok) {
      const allDaySuccessor = allDay.read(
        (allDayResult.value as unknown as { successorEventId: CalendarEventId }).successorEventId,
      );
      expect(allDaySuccessor).toMatchObject({
        ok: true,
        value: { start: { date: "2026-01-07" }, end: { date: "2026-01-09" } },
      });
    }
    allDay.close();
  });
  ...
  it("rejects timed anchor overlap atomically and preserves prefix ownership for a later anchor", () => {
    ...
    const past = {
      kind: "timed",
      instant: "2026-01-05T14:00:00.000Z",
      timeZoneId: "America/Toronto",
    } as unknown as CalendarTime;
    const future = {
      kind: "timed",
    ...
    expect(
      persistence.transaction((tx) =>
        tx.replaceChildren(eventId, {
          exceptions: [{ occurrence: past, title: "past" }],
          exclusions: [future],
          tags: ["keep"],
        }),
      ),
    ...
    for (const start of ["2026-01-06T09:00:00-05:00", "2026-01-05T09:00:00-05:00"]) {
      ...
      expect(
        persistence.readBaseCandidates(10, {
    ...
    const later = mutateCalendarEvent(
      mutation({
        operation: "update",
        eventId,
        applyTo: "this_and_following",
    ...
    expect(later).toEqual({
      ok: true,
      value: expect.objectContaining({ eventId, successorEventId: expect.any(String) }),
    });
    if (later.ok) {
      expect(persistence.read(eventId)).toMatchObject({
        ok: true,
        value: { revision: 2, exceptions: [{ title: "past" }], exclusions: [] },
      });
      const successor = persistence.read(
        (later.value as unknown as { successorEventId: CalendarEventId }).successorEventId,
      );
      expect(successor).toMatchObject({
        ok: true,
        value: {
          start: { instant: "2026-01-08T14:00:00.000Z" },
          exclusions: [{ instant: "2026-01-09T14:00:00.000Z" }],
        },
      });
    }
    persistence.close();
  ...
  it("rejects all-day anchor overlap atomically and accepts a later date anchor", () => {
    ...
    if (later.ok) {
      ...
      expect(
        persistence.read((later.value as unknown as { successorEventId: CalendarEventId }).successorEventId),
      ).toMatchObject({ ok: true, value: { start: { date: "2026-01-08" }, exclusions: [{ date: "2026-01-09" }] } });
    }
    persistence.close();
  });
  ...
  it("rejects child access to hidden effective targets and unavailable split slots before mutation", () => {
    ...
    const cancelled = {
      kind: "timed",
      instant: "2026-01-07T14:00:00.000Z",
      timeZoneId: "America/Toronto",
    } as unknown as CalendarTime;
    const excluded = {
      kind: "timed",
    ...
    expect(
      writer.transaction((tx) =>
        tx.replaceChildren(eventId, {
          exceptions: [
            { occurrence: hidden, visibility: "adults" },
            { occurrence: cancelled, cancelled: true },
          ],
          exclusions: [excluded],
          tags: [],
        }),
  ...
  it("splits COUNT recurrence state at the selected original slot", () => {
        ...
        tx.replaceChildren(eventId, {
          exceptions: [
            { occurrence: cancelled, cancelled: true },
            {
              ...
              start: {
                kind: "timed",
                instant: "2026-01-10T14:00:00.000Z",
                timeZoneId: "America/Toronto",
              } as unknown as CalendarTime,
            },
          ],
    ...
    expect(updated).toEqual({
      ...
      value: expect.objectContaining({
        ...
        eventId,
        successorEventId: expect.any(String),
        resultingRevision: 2,
      }),
    });
    ...
    if (prefix.ok && successor.ok && prefix.value.recurrence && successor.value.recurrence) {
      ...
      const successorSlots = enumerateGeneratedSlots({
        start: successor.value.start,
        recurrence: successor.value.recurrence,
      });
  ...
  it("replaces the whole segment at the first slot without an empty prefix", () => {
    const persistence = openCalendarPersistence(cap(root()), config);
    const created = createCalendarEvent(
      createInput({ recurrence: { frequency: "weekly", weekdays: ["monday"], count: 2 } }),
      persistence,
      config,
    );
    expect(created.ok).toBe(true);
    ...
    const result = mutateCalendarEvent(
      mutation({
        ...
        eventId: created.value.eventId,
        applyTo: "this_and_following",
        originalStart: "2026-01-05T14:00:00.000Z",
    ...
    expect(result).toEqual({
      ok: true,
      value: expect.objectContaining({ eventId: expect.any(String), resultingRevision: 1 }),
    });
    if (!result.ok) return;
    expect(result.value.eventId).not.toBe(created.value.eventId);
    expect(persistence.read(created.value.eventId as CalendarEventId)).toEqual({ ok: false, error: "not-found" });
    expect(persistence.read(result.value.eventId as CalendarEventId)).toMatchObject({
      ok: true,
      value: { title: "replacement", revision: 1 },
    });
    persistence.close();
  });
  ...
  it("rolls back the prefix and generated successor on a split failure", () => {
    ...
    const failing = openCalendarPersistence(cap(base), config, {
      fault: (operation) => {
        if (operation === "insert-successor") throw new Error("synthetic fault");
      },
    });
    const result = mutateCalendarEvent(
      mutation({
    ...
    expect(failing.read(created.value.eventId as CalendarEventId)).toMatchObject({
      ok: true,
      value: { title: "synthetic event", revision: 1, recurrence: { rule: { count: 3 } } },
    });
    failing.close();
  });
  ...
  it("truncates a COUNT series by generated identity and retains only past child state", () => {
    const persistence = openCalendarPersistence(cap(root()), config);
    const created = createCalendarEvent(
      createInput({ recurrence: { frequency: "daily", count: 5 }, tags: ["keep"] }),
      persistence,
      config,
    );
    expect(created.ok).toBe(true);
    ...
    expect(result).toEqual({
      ok: true,
      value: { operation: "delete", appliedTo: "this_and_following", eventId, resultingRevision: 2 },
    } as never);
    expect(persistence.read(eventId)).toMatchObject({
      ok: true,
      value: {
    ...
    const query = createCalendarQueryService({ private: persistence, role: "adult", config });
    const listed = query.listComplete({ from: "2026-01-01", to: "2026-01-31" });
    expect(listed.ok && listed.value.map((value) => value.originalStart)).toEqual([
      "2026-01-05T14:00:00.000Z",
    ] as never);
    persistence.close();
  });
  ...
  it("truncates UNTIL recurrence at the previous generated slot", () => {
    const persistence = openCalendarPersistence(cap(root()), config);
    const created = createCalendarEvent(
      createInput({ recurrence: { frequency: "weekly", weekdays: ["monday"], until: "2026-01-19" as never } }),
      persistence,
      config,
    );
    expect(created.ok).toBe(true);
    ...
    const result = mutateCalendarEvent(
      mutation({
        ...
        eventId,
        applyTo: "this_and_following",
        originalStart: "2026-01-12T14:00:00.000Z",
      }),
      persistence,
      config,
    );
    expect(result).toEqual({
      ok: true,
      value: expect.objectContaining({ appliedTo: "this_and_following", resultingRevision: 2 }),
    });
  ...
  it("deletes the complete segment at the first slot and leaves no successor", () => {
    ...
    expect(persistence.readBaseCandidates(10, { allDayFrom: "2026-01-01", allDayTo: "2026-01-31" })).toEqual({
      ok: true,
      value: { ids: [], overflow: false },
    });
    persistence.close();
  });
  ...
  it("rejects unavailable or unauthorized targets and rolls back failed truncation", () => {
    ...
    expect(cancelled.ok).toBe(true);
    const unavailable = mutateCalendarEvent(
      mutation({
        operation: "delete",
        eventId,
        applyTo: "this_and_following",
    ...
    expect(writer.read(eventId)).toMatchObject({
      ok: true,
      value: { revision: 2, recurrence: { rule: { count: 3 } } },
    });
    writer.close();
    ...
    const rolledBack = mutateCalendarEvent(
      mutation({
        operation: "delete",
        eventId,
        applyTo: "this_and_following",
    ...
    expect(failing.read(eventId)).toMatchObject({
      ok: true,
      value: { revision: 2, recurrence: { rule: { count: 3 } } },
    });
    failing.close();
  ...
  it("rejects invalid scopes and aborts before writing without logging content", () => {
    ...
    expect(
      persistence.readBaseCandidates(1, {
