...
// transient poll failure is emitted but the loop keeps retrying. The collector
// (VM) owns the lifecycle — cancelling collection (navigate away / QR expiry)
// stops the poll.
// ---------------------------------------------------------------------------
...
class DevicesUseCases(
    ...
    fun pollLinkStatus(intervalMs: Long): Flow<SentientResult<SignalLinkStatusResponse>> = flow {
        ...
        while (true) {
            ...
            emit(r)
            if (r is SentientResult.Success && isTerminalLinkState(r.data)) {
