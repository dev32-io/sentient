// ---------------------------------------------------------------------------
// DuplexEngine.ios.kt — ONE AVAudioEngine serving the full-duplex (Continuous) path.
//
// This is the ORIGINAL IosVoiceAudio machinery, byte-for-byte: the proven iOS audio
// snippets (mic tap, playback player, full-duplex VoIP session, ObjCExceptionGuard)
// consolidated into one engine. configure(mic, playback, rate) diffs voiceAudioGraph
// against the last-applied graph, then stop → reconfigure (tap/player/VPIO) → start.
// VPIO toggled ONLY in the mic+playback cell, only inside a stop → always starts on a
// clean session (kills StartIO-on-dirty-session). Player always connected to
// mainMixerNode → never starts disconnected (kills the 2633b4e crash class).
//
// S4 SPLIT: extracted out of IosVoiceAudio (now a path-routing facade) so the Manual
// (hold-mode) MediaPlayback / MicCapture engines can live beside it WITHOUT touching
// this proven duplex path. The facade owns the single [state] StateFlow + [micCh]
// channel and injects them here; every log message + tag + session shape is unchanged
// (log-trail continuity). Used for ALL cells when path == Duplex.
//
...
// the "capture-level" log below. 1.0f = no boost. Mirror of MicCaptureEngine's copy
// (spec §7.2: MIC_CAPTURE_GAIN stays 4.25 on BOTH capture paths) — keep in lockstep.
...
// the voice-tuned route, well below media-route loudness. This gain lifts the floor;
// peaks hard-clamp to [-1,1] in pcm16ToFloatBuffer, so past ~×6 loud peaks clip audibly.
// Do NOT normalize per-voice — quiet voices are a character trait. 1.0f = no boost.
// Mirror of MediaPlaybackEngine's copy (spec §7.2: PLAYBACK_MAKEUP_GAIN = 5.0 on BOTH
// playback paths, was duplex 5.5) — keep in lockstep.
private const val PLAYBACK_MAKEUP_GAIN = 5.0f

/**
 * ONE AVAudioEngine serving the Continuous (full-duplex) path. session = .playAndRecord +
 ...
 *
 * The single [state] StateFlow + [micCh] channel are OWNED by the [IosVoiceAudio] facade
 * and injected here — this engine feeds them (spec §7: one state + one micFrames across
 * all engines).
 */
internal class DuplexEngine(
    ...
    suspend fun configure(mic: Boolean, playback: Boolean, playbackRateHz: Int) {
        ...
        state.value = state.value.copy(phase = Phase.Configuring, micActive = mic, playbackActive = playback)
        runCatching {
            engine.stop()
            if (desired.running) {
    ...
    /**
     * Tear the duplex engine to a clean idle baseline WITHOUT writing state (the facade
     * owns the resulting state after a path switch / shutdown). Used only when the facade
     * switches AWAY from Duplex to a Manual engine or on shutdown; the normal Continuous→
     * Idle transition goes through configure(false, false) (byte-identical to today).
     */
    fun teardown() {
    ...
    private fun failReset(mic: Boolean, playback: Boolean, reason: String) {
        ...
        playbackEpoch.incrementAndGet()
        current = voiceAudioGraph(mic = false, playback = false)
        state.value = VoiceAudioState(Phase.Error, micActive = mic, playbackActive = playback, errorReason = reason)
