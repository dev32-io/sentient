...
//
// Note: the body is sent as an in-memory String; fine for the typical few-MB
// log, but a 50 MB ceiling file would be held fully in memory (acceptable
// tradeoff for now; streaming is a future option).
// ---------------------------------------------------------------------------
...
// Segment-relative path — baseUrl already ends at /api/v1.
// Composed wire URL: https://host/api/v1/diagnostics/logs
private const val PATH_DIAGNOSTICS_LOGS = "/diagnostics/logs"
 ...
 *
 * @param httpClient Ktor HttpClient with platform engine (OkHttp / Darwin).
...
open class VitalsUploader(
     ...
     * @return Server-assigned ref string on success, `""` if the upload
     *   succeeded but no ref could be parsed from the response body, or
     *   `null` if the upload failed (transport error or non-2xx status).
     *   Callers MUST treat a non-null return as success and MUST NOT retry.
     */
    suspend fun upload(fileName: String, body: String, onProgress: (Double) -> Unit): String? {
        log.debug("upload.start", mapOf("fileName" to fileName, "chars" to body.length))
        return try {
            val resp = httpClient.post("$baseUrl$PATH_DIAGNOSTICS_LOGS") {
                headers {
            ...
            val ref = REF_PATTERN.find(resp.bodyAsText())?.groupValues?.get(1)
            if (ref == null) {
                log.warn("upload.ref-missing", mapOf("status" to resp.status.value))
                return ""   // upload succeeded; ref just couldn't be parsed — non-null so caller shows success
            }
    ...
    companion object {
        private val REF_PATTERN = Regex("\"ref\"\\s*:\\s*\"([^\"]+)\"")
    }
}
