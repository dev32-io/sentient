...
5. The platform wrapper applies its reviewed tolerance (Web `0.56`, iOS `0.15`, Android `0.56`) and the shared `0.2%` visible-difference gate. Use the underlying pairwise command without `--max-diff-percentage` only for explicit report-only diagnostics.
