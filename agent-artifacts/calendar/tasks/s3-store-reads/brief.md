# Task Brief: CalendarStore get/list/filter with visibility and serving-timezone recurrence expansion
...
- Implement list(filter) expanding recurring events via expandRecurrence in the serving tz, applying EXDATE skips and exception overrides
...
- Typed get/list results including not-found
...
- 2. Implement list(filter) selecting base events overlapping [start, end] and expanding recurring events via expandRecurrence in the configured serving tz, applying EXDATE skips and exception overrides.
- 3. Apply group, tags, and importance filters and role-based visibility filtering.
- 4. Return typed not-found for absent ids.
- 5. Add tests for COUNT=10 BYDAY=MO,FR expansion, EXDATE/exception application, each filter dimension, child/guest adults-event omission, admin/adult visibility, serving-tz expansion, and not-found.
...
- expandRecurrence runs in the configured serving timezone (America/Vancouver default) over UTC-stored instants; the store read path applies EXDATE/exception overrides.
- list/filter selects base events overlapping [start, end] and expands recurring ones within the window.
