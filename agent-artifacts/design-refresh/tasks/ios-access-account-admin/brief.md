...
- 3. Preserve login user loading/empty/actionable error, user selection, PIN entry/delete/back/submitting, invalid PIN/network/server errors, and authenticated identity. Never persist/display/log PINs or credentials. Consume the startup task's readiness contract; do not create another readiness clock/model.
...
- 9. Add focused tests/previews for all branches, privacy redaction, admin/non-admin behavior, setup validation/probe failure, login readiness/error presentation, update states, account restore/logout, member mutations, secret presence/apply, and native large-text reachability.

## Integration Expectation

Deliver this contribution for integration in stage ios-final-products.

## Context

- Owned paths are ios/App/Auth Login/user/PIN views (the startup task owns any AuthViewModel readiness additions), SDK/BackendSetupView*.swift and LocalNetworkPrimer presentation, Update/**, Settings/Account/**, Settings/Members/**, Settings/Secrets/**, plus owned tests/previews.
- Root startup/readiness and NavigationStack ownership are provided by ios-identity-startup-history; SettingsView route composition is provided by ios-settings-system.
...
- Startup clock/Rive/History/readiness-model implementation
...
- Consumes native common components, existing ViewModels/services, and the readiness contract from ios-identity-startup-history.
