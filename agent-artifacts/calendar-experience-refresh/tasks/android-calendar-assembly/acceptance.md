...
- Android matches the exact mobile reference at logical 390x844 and 430x932 for supported behavior and documents only approved corrections.
...
- The final E2E agent can drive the committed direct-Maestro steps sequentially with unique fixtures for Android portions of CAL-UX-001..014.
- Screenshots compare production to `sentient-design/design/mobile/calendar.html` and behavior to `sentient-design/components/mobile/sentient-mobile.js` at exact logical 390x844 and 430x932; semantic evidence records focus/target/safe-area/reduced-motion checks.
- Residual risk notes that semantic inspection is not an actual TalkBack session.
