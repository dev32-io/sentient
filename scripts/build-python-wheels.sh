...
build() {
  ...
  # downstream. The lock is generated by the recipe in a sibling lock's header
  # (uv pip freeze against the tested .venv → uv pip compile → this script rehashes).
  if [[ ! -f "$REQ/$service.lock" ]]; then
    echo "ERROR: missing $REQ/$service.lock — generate it before building $service wheels." >&2
    echo "       Recipe: see the header of $REQ/whisper-stt.lock (uv pip freeze --python" >&2
    echo "       $requirements's sibling .venv, then uv pip compile, then re-run this script)." >&2
    exit 1
  fi
