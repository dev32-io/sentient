...
- Auth: PASETO v4.local for browser session tokens; service-to-service auth via the shared-token model in the `sentient-auth` package. Prompt-injection scanning + a risk accumulator in `gateway/src/security/` — those two files are all it contains. Tool-call gating is NOT among them and shares nothing with them: it is per-tool permissions (`allow|ask|deny|off`) over a role gate (`canExecute(role, tier)`), resolved at the tool boundary in `gateway/src/tools/`. The `mcp-policy.yaml` rules engine is deleted.
