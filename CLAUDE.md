...
- Auth: PASETO v4.local for browser session tokens; service-to-service auth via the shared-token model in the `sentient-auth` package. `gateway/src/security/` holds the injection scanner + risk accumulator + the inbound scanning boundary: MCP tool results, background-completion payloads, and skill bodies cross one gate (`inbound-gate.ts`) that runs `scanContent` (normalization + bilingual pattern bank + structural tool-envelope detection) and feeds `RiskAccumulator`; findings annotate and raise PDP risk rather than block, escalating a side-effecting tool to `confirm` at high risk. The outbound delegation prompt is scanned too, but not through that gate: it runs the same `scanContent` DIRECTLY via `prompt-classifier` — no gate, no `RiskAccumulator` feed. Tool-call gating is NOT among them and shares nothing with them: it is per-tool permissions (`allow|ask|deny|off`) over a role gate (`canExecute(role, tier)`), resolved at the tool boundary in `gateway/src/tools/`. The `mcp-policy.yaml` rules engine is deleted.
