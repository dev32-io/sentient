...
- **Seed:** the orchestrator calls `ResumeCursorPersistence.seedIfEmpty()` at the wire moment (in `resumeParams()`, before building the `stream.resume` frame): if the in-memory cursor is empty and a conversation is anchored, it `store.load(conversationId)`s and resets the in-memory `ResumeCursor` to the persisted `{epoch, lastSeq}`. The reconnect then carries a non-zero cursor → `recovered:true` replays the gap; `recovered:false` means the buffer expired.
...
- **delete-THEN-anchor order in `collectSwitches`:** `deleteConversation` runs BEFORE `currentConversationId.value = id`. This prevents `flatMapLatest` from re-subscribing to the old stale rows between the delete and the re-anchor — no intermediate phantom paint.
