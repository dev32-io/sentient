...
- **`12-permission-confirm` ACTUATES THE REAL HOUSE, and it is tagged `chat`** — it sends "turn on the kitchen light" and taps Allow. There is no tag or flag that excludes a single flow, so `--tags chat` (the tag set that selects the chat-stream flows) drags it in. The local stack talks to the user's real Home Assistant; there is no dev/prod split for physical state. When you only need the chat-stream flows, invoke Maestro directly on the explicit file list instead — `~/.maestro/bin/maestro --device <id> test -e QA_USER_ID=<id> flows/<plat>/01-send-stream.yaml flows/<plat>/01b-reply-survives-relaunch.yaml --exclude-tags helper` — which is still ONE warm invocation. (A `device-actuating` behavior tag + a runner exclude would fix this properly.)
...
### T12 — Composer task strip clears at the turn boundary (web + Android + iOS)
**Scenario:** Live tool rows render as pills at the top of the composer, INSIDE its border — never attached to a chat bubble — transition running → done, and the strip empties when the turn ends. A background `delegateTask` row instead OUTLIVES its turn and leaves only when the delegation settles.
**Why added:** Regression guard for the server-authoritative full-state `tasklist.state` frame (2026-08-06). The gateway owns which rows exist and how long each lives; clients are dumb renderers. Foreground rows die with their turn, background rows do not.
**Steps:** fresh chat → send a two-or-more-tool read-only prompt (T11's prompt works) → assert pills appear in the composer and none on any bubble → wait for `turn.completed` → assert the strip is empty and the composer returns to its idle height.
**Expected user-visible:** strip inside the composer (web: `.tool-strip` is `.composer`'s first child and its rect is contained by the composer's; Android/iOS: `task-strip`); 0 pills on bubbles; strip gone after the turn; no layout jump (web desktop measured 163 px during → 122 px idle).
**Expected log trail (gateway):** `task-list.tool-update` per transition with a matching `turn-emitter.task-list count=<n>`; at the boundary `task-list.turn-ended rows=0` + `turn-emitter.task-list turnId=null count=0`. Clients: `connector.task-list state count=<n>` … `state turnId=- count=0`.
**Background variant (`delegateTask`):** the row survives `task-list.turn-ended rows=1` (`count=1` with `turnId=null`), survives the NEXT turn's `task-list.turn-started rows=1`, and leaves on `task-list.delegation-settled status="done" rows=0`. `delegateTask` is `confirm`-tier, so a permission dialog gates it. **Only ever drive it with a purely informational delegated prompt**, and check the log for what the delegate actually called (`sessionId="delegated:<userId>"` `tool-broker.pdp.decision` lines) before calling the case green — the delegated allow-tier still includes `ma_playback`/`ma_play_media`/`ma_volume`, so a loosely-worded task CAN actuate the house.
