...
### KNOWN BUG (Task 23, 2026-08-09) — journal scan false-positive fails the whole dream
The dreamer's journal NARRATIVE is LLM prose summarizing the sessions, and a faithful summary naturally says "the user said…" / "the user requested…" / "the user confirmed…". The injection scanner's `context_manipulation` pattern (`injection-scanner.ts:148`, `/\bthe\s+user\s+(?:has\s+)?(?:been\s+)?(approved|authenticated|authorized|confirmed|said|requested)\b/i`) matches those at `suspicious` severity. The journal is written via `MemoryStore.write` (channel `memory_body`), whose WRITE-time scan HARD-REFUSES at suspicious+ (unlike the read-time inbound-gate, which only annotates) → `store.write.scan-rejected relPath="journal/<date>.md" categories=context_manipulation` → `episode-writer.journal.refused error="scan_rejected"` → `dreamer.run.failed reason="journal-refused"`. MEMORY.md was already mutated by the reconciler (the ops use terse "User is/decided…" lines that PASS the scan), but the run is reported FAILED, the mark does NOT advance (→ re-dream + re-spend every boot), and the index flush never runs (→ episodes never indexed; `spark.withheld reason="empty"` on recall). MEMORY.md-mutation + prompt-render sub-path is verified working; the journal/`run.ok` oracle is blocked. S3a dodged this only because its summaries happened to use "asked"/"mentioned" (not in the pattern) with ops=0. **Product decision needed** (exempt dreamer-journal from the write-time bank / degrade journal-refusal to `ok-episodic-only` like the reduce-refusal path / narrow the `the user (said|requested|…)` pattern). Until fixed, the FULL row cannot go green.

### memory-mobile (Part B) — DEFERRED, first-driver groundwork
The first native memory coverage (`60-memory-write`, `61-memory-recall`) was deferred at Task 23 (stopped on the Part A FAIL), but the setup is scoped for the next driver: target **iOS** (the confirm alert `permission-allow`/`permission-deny` works, `ios/12-permission-confirm.yaml` green), NOT Android (D13: `testTagsAsResourceId` not on the Compose `AlertDialog` → `chat-permission-allow` unresolvable, confirm dialog RED). `memory_write` is confirm-tier so `60-memory-write` must tap `permission-allow` after send then assert the confirmation bubble — and it PERSISTS to the user's real MEMORY.md, so it needs a teardown/restore (unlike `46-memory-cap`, which discards). appId `io.dev32.sentient.debug`; login `_helpers/login.yaml`; chat ids `composer-input`/`chat-send`/`assistant-bubble`/`new-chat`.
