...
Cases for the S2 deep-memory system (`gateway/src/memory/memory-retriever.ts`, `deep-memory-wiring.ts`, `gateway/src/tools/memory-tools.ts` `memory_recall`, `capabilityServices/DeepMemoryService/`). First driven 2026-08-09 (plan `2026-08-08-memory-system.md`, Task 16), web-only.
...
**Seeding (exercises the outbox):** a `memory_write` through chat writes MEMORY.md AND syncs it to the index — `index-sync.enqueue-file | file="MEMORY.md"` → `index-sync.flush.ok | upserted=1`. NB the whole MEMORY.md is ONE `file-section` index entry (not per-line); editing it leaves the pre-edit section entry ORPHANED in the index (content-hashed id; both stay `active`) — recall/spark then return an extra stale hit. `gpt-oss:20b` often chooses `scope=family`/a `topics/<slug>` target for a "remember" — steer to `scope=private, target=MEMORY.md` if the private recall index is what you need.
...
**Oracle:** `[memory:retriever] memory-retriever.spark.hit | turnId= similarity= orderScore= kind="file-section"` (DEBUG — see gotcha). Per-turn spark ran before the first provider call and injected the snippet.

### spark-empty — ⚠️ below-threshold oracle UNREACHABLE with the shipped config
**Scenario:** unrelated topic in a fresh session; expect NO memory reference and `spark.withheld | reason="below-threshold"`.
**Finding (Task 16):** the user-visible half holds — the model does not leak the seeded snippet on an unrelated turn — but the pinned log oracle does NOT fire. `orchestrator.memory.spark.min_similarity` is `0.60`, well below the empirical cosine floor of `multilingual-e5-small` (~0.74 even for gibberish/foreign-language queries; measured 0.74–0.79 against a populated scope). `gateAndOrder` gates on `similarity >= min_similarity`, so against any non-empty index EVERY turn `spark.hit`s and NOTHING is withheld for below-threshold (the only reachable withhold is `reason="empty"`, i.e. a zero-entry scope). The relevance gate is effectively a no-op; model-side judgement, not the gate, is what suppresses the irrelevant snippet. **Needs a product decision** (retune `min_similarity` to the e5 distribution, e.g. ~0.80, or drop the below-threshold expectation).
