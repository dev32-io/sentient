...
## Household / family scope (S4) — shared `family` scope + audience filtering

Cases for the S4 household scope (`gateway/src/bootstrap/phase-services.ts` `buildSessionMemory`, `memory-prompt.ts` household render, `memory-retriever.ts` child audience filter, `index-sync.ts` `@adults` parse). The shared MEMORY.md lives at `<sharedDataRoot>/<householdId>/memory/MEMORY.md` — with `access.shared_data_root` **unset**, sharedDataRoot derives to `join(dirname(user_data_root), "shared")` = `~/.sentient/gateway/shared`, and the live householdId is `home`, so the file is `~/.sentient/gateway/shared/home/memory/MEMORY.md` and the scope id is `household:home`. EVERY session (adult AND child) mints a `memory-household` grant and renders the family envelope; `memory.prompt.rendered … scopes="private,family"` is the S4 signature (S1/S2/S3 were `scopes="private"`). First driven 2026-08-09 (Task 26), web-only, desktop 1280×900. Requires the S1 provider switch; deep-memory bring-up needed ONLY for the spark-path `audience.filtered` corroboration (see gotcha).

**Deep-memory data_root gotcha (family spark path).** The household index registers at `<sharedDataRoot>/home/memory/deep-memory/index.db` = under `~/.sentient/gateway/shared`, but the S2 `~/.sentient/deep-memory/config/config.yaml` `storage.data_root` is `~/.sentient/gateway/users` — so register-scope REFUSES the family scope (`path_outside_data_root`) and the family spark never runs. To exercise `audience.filtered` (spark path), broaden `storage.data_root` to `~/.sentient/gateway` (parent of BOTH `users/` and `shared/`); the private index `users/<uid>/deep-memory/index.db` still resolves inside it. Broadening also relocates `<data_root>/scope-registry.json` to the gateway root — delete that orphan on teardown and revert data_root. **family-scope does NOT need this** (its oracle is the file-based prompt render); only `audience.filtered` does.

**Child-role gotcha.** The webui `MembersPane` exposes only Admin/Member (`setIsAdmin`) — NO child selector. Create a child directly: `POST /api/v1/admin/users` (browser `fetch` with the admin token from `localStorage['sentient:auth'].token`) with `{displayName, pin, role:"child", profile}` — the body needs a FULL valid `profileV1` minus `userId`/`schemaVersion` (mirror an existing user's `profile.json`, set `model:{provider:"custom",id:"gpt-oss:20b"}`). Returns 201 with the new `userId`; PIN-login as the child. `principal.role==="child"` is what drives the audience filter. Delete the created child on teardown.

**ADULT_ROLES = {admin, adult}.** An admin (e.g. Ada) counts as an adult writer/reader; only `child` is audience-filtered. Cross-check both ways in one run: after Ada (adult) writes an `@adults` fact, an adult reader (MemTestB) SEES it and the child (Kiddo) does NOT.

### family-scope
**Scenario:** an adult (A) writes a household fact via `memory_write(scope="family", op="append", target="MEMORY.md")`; a SECOND adult (B) opens a NEW session and asks about it → B knows it. Desktop 1280×900.
**Steer to the CORE, not a topic file (critical).** `gpt-oss:20b` routes a "family" remember to `target="topics/<slug>"` by default — verify in the confirm dialog and DENY if it does. The prompt render includes the family core MEMORY.md **body** in full but topics only as a one-line **index** (`- name — description`), so a fact in a topic file will NOT surface cleanly in B's render. Re-prompt explicitly: `scope="family", op="append", target="MEMORY.md"`, "NOT a topics file". A DENY poisons the conversation (the model reads the deny as "don't retry") — start a FRESH chat for the explicit re-prompt.
**Oracle:** A's write → `memory-tools.write.ok | scope="family" target="MEMORY.md"` + `index-sync.flush.ok upserted=1`; on-disk `~/.sentient/gateway/shared/home/memory/MEMORY.md` gains the line. B's NEW session → `memory.prompt.rendered | userId="<B>" scopes="private,family"` (chars grow from B's private baseline), and B's reply names the fact (B may also fire `memory_read scope="family" read.ok` — cross-member family read, also fine). Verified live: A=Ada wrote "We're going to Tahoe in December.", B=MemTestB's reply named "trip to Tahoe in December", B render `scopes="private,family"`.

### family-audience
**Scenario:** A (adult) writes a family fact line ending `@adults`; a `child`-role user's session asks a topically-adjacent question → the child NEVER surfaces it (but DOES see non-tagged family lines). Desktop 1280×900.
**Oracle:** the child's family block drops the `@adults` line at render (render logs chars+scopes only, never content — so the child's `memory.prompt.rendered scopes="private,family"` chars EQUAL the 1-non-tagged-line baseline, i.e. the tagged line contributes nothing). If the family scope is registered (see data_root gotcha) and the child's query sparks it, the direct spark-path oracle fires: `[memory:retriever] memory-retriever.audience.filtered | turnId= kind="file-section"` (DEBUG — needs `logging.level_overrides: { sentient:memory:retriever: debug }` in the REPO `gateway/config.yaml`, the one the gateway actually loads in dev; restart to pick it up). The child's reply must not mention the fact. Verified live: Ada wrote "There is a surprise birthday party being planned for the kids in December. @adults"; child Kiddo's reply "No one's planning a surprise party right now" (filtered) while still naming the non-tagged Tahoe trip; `audience.filtered kind="file-section"` fired. Cross-check: adult MemTestB and admin Ada BOTH saw the surprise party in the same run — filtering is role-scoped, not global.

**Config-path trap (cost a wasted edit).** In dev the gateway loads the **REPO** `gateway/config.yaml` (`cd gateway && bun --watch src/main.ts`, no `GATEWAY_CONFIG_PATH` → `startup-config.ts` resolves `gatewayRoot/config.yaml`). `~/.sentient/gateway/config/config.yaml` is the docker-addon bind-mount dir (`HOST_CONFIG_DIR`), NOT the gateway's own config — editing the `logging.level_overrides` there is a no-op. Put the retriever-debug override in the repo `gateway/config.yaml` and revert on teardown.
