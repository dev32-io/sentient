...
### memory-mobile (Part B, iOS) — flows authored + run-attempted; BLOCKED on a client-send issue (Task 23)
`ios/60-memory-write.yaml` + `ios/61-memory-recall.yaml` (tags `[memory, chat]`, `_helpers/login.yaml` subflow, QA user Ada `u_0417d3b0`) are authored and committed. Batch-run via `./qa/mobile/run-e2e.sh ios --tags memory` (the runner appends tag-matching non-CANONICAL_ORDER files, so new files ARE selected). **Both FAILED at the LLM-gated assertion, root-caused to the mobile CLIENT, not the flows or memory:** the maestro commands log shows EVERY tap COMPLETED (login → new-chat → composer → `inputText` → `chat-send`), but the gateway logged **zero `stream-start`** and the sessions stayed `session-configured … hasRuntime=false draft=true` — i.e. `chat-send` tapped but never submitted a `text.input` turn. Reproduced across 2 clean runs (delta=0 stream-start); not model latency (a slow turn still logs `stream-start`). The debug app used was a STALE build (`ios/build/Build/Products/…/SentientApp.app`, ~12 days old, predates the memory branch). Next driver: rebuild the app (`./scripts/ios-setup.sh`) and re-run; the flows and selectors are correct (they mirror the passing `01-send-stream.yaml`). **Tooling gotcha hit:** using the Maestro **MCP** server (2.7.0) for `inspect_screen` and then the **CLI** runner (`~/.maestro/bin/maestro`) triggers the "two Maestro versions fight over the simulator-server" bug (`Failed to connect to /127.0.0.1:<port>`) — `pkill -f "maestro.cli.AppKt mcp"; pkill -f simulator-server` before switching to the CLI runner. **Android stays D13-RED** regardless: `testTagsAsResourceId` is not on the Compose `AlertDialog`, so `chat-permission-allow` never resolves — the confirm-tier `memory_write` alert is untappable; run iOS only until D13 is fixed.
