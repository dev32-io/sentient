"""WebSocket entry point for the localSTT PoC service.
...
Wire protocol (server ↔ browser):
  ...
  Server → Client
    - Text JSON ``{"type": "ready", "sampleRate": 16000, ...}``
    - Text JSON ``{"type": "vad_start", "turnIdx": ...}``
    - Text JSON ``{"type": "vad_end", "turnIdx": ...}``
    - Text JSON ``{"type": "smart_turn_eval", ...}``
    - Text JSON ``{"type": "turn_continuing", ...}``
    - Text JSON ``{"type": "turn_complete", ...}`` + one binary WAV frame
    - Text JSON ``{"type": "transcript_ready", "turnIdx": ..., "text": ...,
                    "language": ..., "emotion": ..., "event": ..., "decodeMs": ...}``
"""
...
log = logging.getLogger("localstt")
...
HOST = os.environ.get("LOCALSTT_HOST", "0.0.0.0")
PORT = _env_int("LOCALSTT_PORT", 8766)
MODEL_DIR = _env_path("LOCALSTT_MODEL_DIR", "/app/models")
LOG_DIR = _env_path("LOCALSTT_LOG_DIR", "/app/logs")
RECORDING_DIR = _env_path("LOCALSTT_RECORDING_DIR", "/app/recordings")
METRICS_INTERVAL_MS = _env_int("LOCALSTT_METRICS_INTERVAL_MS", 1000)

SMART_TURN_DIR = MODEL_DIR / "smart-turn"
SENSE_VOICE_DIR = MODEL_DIR / "sense-voice"
...
class Server:
    ...
    def __init__(self) -> None:
        ...
        self._sense_voice: SenseVoice | None = None
    ...
    def load_models(self) -> None:
        ...
        self._smart_turn = SmartTurn(SMART_TURN_DIR)
        log.info("Smart-Turn ready: %s", self._smart_turn.model_path)

        log.info("loading SenseVoice-Small int8 ONNX from %s", SENSE_VOICE_DIR)
        self._sense_voice = SenseVoice(SENSE_VOICE_DIR)
        log.info("SenseVoice ready: %s", self._sense_voice.model_path)

        self._service_log.log(
            "models.loaded",
            silero_backend="torch_jit",
            smart_turn_path=str(self._smart_turn.model_path),
            sense_voice_path=str(self._sense_voice.model_path),
        )
    ...
    async def handle(self, ws: ServerConnection) -> None:
        ...
        assert self._sense_voice is not None, "load_models() must run before serve()"
