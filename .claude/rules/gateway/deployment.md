...
- The gateway is a native host process, not a container. Docker addons are supervised dependencies; Hermes is a one-shot exec, never a managed service.
...
- MCP servers are one container per service and are reached by the host gateway over loopback. Never configure gateway access through Docker DNS or `host.docker.internal`.
