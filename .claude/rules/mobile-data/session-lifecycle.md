...
- Lifecycle ops on the connection scope: open (connect in the background; the UI never blocks — optimistic send bridges the gap), pause (background entry: KEEP the socket — the gateway holds the per-user session + ACP wire on detach and has no server ping/timeout, so a brief background survives on the SAME socket with no reload/reconnect/audio-teardown; do NOT proactively drop), resume (foreground: one-shot liveness probe — ping then reconnect + session.switch-resume ONLY if no pong returns in the window; never a periodic heartbeat), close (auth end: disconnect + cancel the scope).
