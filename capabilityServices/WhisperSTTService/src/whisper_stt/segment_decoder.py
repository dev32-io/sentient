...
placeholders positioned correctly in the transcript we decode each
segment separately, then concatenate per-segment transcripts with
``[pause.N]`` tokens in between.

Cost: N decodes instead of 1. On RPi5 + SenseVoice-Small int8, 4 short
segments take roughly 1.5-2x the wall time of a single whole-turn
decode. For single-segment turns (no pauses) there is zero overhead.

Python note — ``TYPE_CHECKING`` guard:
  The ``if TYPE_CHECKING:`` block at the top runs ONLY when a static
  type checker (mypy, pyright) is analyzing the file — it is completely
  skipped at runtime. We use it to import ``JsonlLogger`` for type
  hints without creating a circular import at runtime (since
  ``event_logger.py`` doesn't depend on us, but we want to type-hint
  it). Kotlin doesn't have this problem because its type system
  resolves everything at compile time regardless of import order.
"""
...
def decode_segments_and_stitch(
    ...
    *,
    ...
    A well-formed turn satisfies ``len(segments) == pause_count + 1``.
    We do not assert it — malformed inputs produce truncated output
    rather than crashing. Defensive, not strict.
    """
    ...
    # Stitch: "seg0 [pause.0] seg1 [pause.1] seg2"
    parts: list[str] = []
