...
inside the Qwen3-TTS model repo itself, loaded automatically when
``ref_audio`` is omitted (see ``engine.py``). The model bytes
...
~/.sentient/local-tts/models/ without knowing HuggingFace's internal
cache layout.
...
# Matches the deployed default seeded into config.yaml by
# deploy/mac-prod/native/local-tts.sh's install step. Used when the
# caller doesn't pass an explicit model_id override.
DEFAULT_MODEL_ID = "mlx-community/Qwen3-TTS-12Hz-0.6B-Base-8bit"
