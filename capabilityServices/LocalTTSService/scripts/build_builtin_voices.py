...
``src/local_tts/voices_library/<slug>/`` (see that module's
...
``LocalTTSService`` project root, e.g.
``capabilityServices/LocalTTSService``). Needs only ``soundfile``
...
from local_tts.pack_meta import META_FILENAME, REF_FILENAME

log = logging.getLogger("local_tts.scripts.build_builtin_voices")

# Qwen3-TTS's reference-clip sample rate — matches engine.py's
...
_LIBRARY_DIR = _SCRIPT_DIR.parent / "src" / "local_tts" / "voices_library"
