...
it consumes the 24 kHz float32 PCM chunks ``QwenEngine.synthesize``
yields (see ``engine.py``) and produces wire-format bytes ready
...
def make_encoder(format: str, target_rate: int) -> AudioEncoder:
    ...
    if format == FORMAT_OPUS:
        from local_tts.encoders.opus_encoder import OpusEncoder

        return OpusEncoder(target_rate)
    if format == FORMAT_PCM:
        from local_tts.encoders.pcm_encoder import PcmEncoder
