...
[`WhisperSTTService`](../WhisperSTTService/). Wraps Qwen3-TTS (MLX) into a
...
> **Status: complete.** `python -m local_tts` loads + validates config,
> loads the MLX Qwen3-TTS model, warms it with one real end-to-end
...
> SIGINT/SIGTERM — see `src/local_tts/__main__.py`. In short: text in,
...
Every tunable constant lives in `~/.sentient/local-tts/config/config.yaml`
(pointed at by the `LOCAL_TTS_CONFIG_PATH` env var). There are zero hardcoded
...
Host paths are **not** in `config.yaml` — they come from `LOCAL_TTS_*`
environment variables, set by the launchd plist (or exported by hand for local dev):

| Env var | Purpose | Default when unset |
|---------|---------|---------------------|
| `LOCAL_TTS_CONFIG_PATH` | Path to `config.yaml` | `~/.sentient/local-tts/config/config.yaml` |
| `LOCAL_TTS_LOG_DIR` | JSONL log output dir | `~/.sentient/local-tts/logs` (or `config.yaml`'s `log_dir` if set) |
| `LOCAL_TTS_VOICE_DIR` | Reference voice-clip dir | `~/.sentient/local-tts/voices` (or `config.yaml`'s `voice_dir` if set) |
...
LOCAL_TTS_CONFIG_PATH=./config/config.example.yaml \
LOCAL_TTS_LOG_DIR=./logs \
uv run python -m local_tts
```
...
see `deploy/mac-prod/native/local-tts.sh` (`install` / `start` / `stop` /
`status` / `logs`).
...
- Structured JSONL logging: `<date>-service.jsonl`, `<date>-metrics.jsonl`,
  ...
  `retention_days`.
- `src/` layout (`src/local_tts/`), `python -m local_tts` entry point via
  `__main__.py`.
