...
``test_local_tts_engine.py`` pattern.
"""
...
def test_create_cleans_up_on_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("local_tts.voice_store.sf.write", _boom)
...
def test_create_then_get_roundtrip(tmp_path: Path) -> None:
    ...
    # Reuse the same bench reference clip as test_local_tts_engine.py (~5.5s,
