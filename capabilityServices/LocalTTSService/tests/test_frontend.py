...
def test_end_to_end_markdown_currency_emoji(policy):
    fe = build_frontend(normalize_enabled=True, normalize_languages=_ALL_LANGS, policy=policy)
...
def test_normalize_disabled_keeps_symbols_but_strips_markdown(policy):
    fe = build_frontend(normalize_enabled=False, normalize_languages=_ALL_LANGS, policy=policy)
