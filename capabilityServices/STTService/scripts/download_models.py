...
# --- SenseVoice-Small int8 (via csukuangfj's sherpa-onnx mirror) ---
...
def _mib(p: Path) -> str:
    """Format a file's size in MiB for log output."""
...
def main() -> None:
    ...
    for filename in SENSE_VOICE_FILES:
        print(f"[download_models] fetching {SENSE_VOICE_REPO}/{filename}")
        local_path = Path(
            hf_hub_download(
        ...
        print(f"[download_models]   -> {local_path} ({_mib(local_path)})")
