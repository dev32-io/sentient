# Chatterbox-TTS — Service Contract

A WebSocket service that turns streamed text into streamed audio. Wraps
Chatterbox-Turbo (MLX, Apple Silicon / Metal) behind a black-box "text in,
...
lock as text synthesis — a `voice.create` in flight blocks (and is
blocked by) every synthesis request on every connection, and vice versa.
...
`ChatterboxEngine.synthesize()` sets voice conditioning by mutating a
process-wide, `lru_cache`'d MLX model singleton's `model._conds` before
generating — concurrent synthesis calls would race on that mutation and
silently produce wrong-voice (or corrupted) audio. `synthesize()` is
also a blocking, synchronous generator on a single GPU, so there is no
throughput benefit to attempting concurrency anyway.
...
(which mutates the same singleton via `prepare_conditionals`) — across
every connection. `voice.list`/`voice.delete`/`ping` never wait on this
...
| Server RAM (steady state)           | Chatterbox-Turbo-8bit weights ≈ 1.4 GB       | Plus warm voice-pack cache; coexists with native Whisper-STT + the Docker gateway stack on the same host. |
