...
behind the same service-facing shape as the sibling ``ChatterboxEngine``
(``chatterbox_mlx.py``): load once, warm once, then stream 24 kHz float32
PCM chunks per synthesis call. This is the multilingual (en/zh via
``lang_code="auto"``) replacement engine landing during the TTS engine
swap (Task 1 of the swap plan) — the package rename
(``chatterbox_tts`` -> ``local_tts``) and the swap-over wiring happen in
later tasks, so this module intentionally lives in the current
``chatterbox_tts`` tree for now; it moves with the package rename.

Voice cloning: unlike Chatterbox's ``conds.safetensors`` +
``prepare_conditionals()`` scheme, Qwen3-TTS clones directly off a
reference wav path (or ``mx.array``) passed as ``ref_audio`` on every
``generate()`` call — there is no separate "prepare" step and no
persisted conditioning object to manage. Passing ``ref_audio=None`` uses
the model's built-in default voice. ``ref_text`` is optional (used only
for the model's in-context-learning clone path) and is intentionally
never passed here — none of this project's reference clips ship a
transcript, and ``ref_audio``-only cloning was verified to produce good
audio on a bench script during evaluation.

Model loading: ``_load_model`` is module-level and ``@lru_cache``'d by
``model_id``, mirroring ``chatterbox_mlx._load_model`` and the sibling
STT service's ``ModelHolder``
...
# real request doesn't pay for lazy Metal-kernel / cache warmup. Mirrors
# chatterbox_mlx.py's warm constants.
...
def _load_model(model_id: str):
    ...
    """
    log.info("qwen_engine.model_load start model_id=%s", model_id)
    t0 = time.monotonic()
    model = load_model(model_id)
    elapsed_ms = (time.monotonic() - t0) * 1000.0
    log.info(
        "qwen_engine.model_load done model_id=%s elapsed_ms=%.1f",
        model_id, elapsed_ms,
    )
    return model
...
def _prepare_chunk(result) -> np.ndarray:
    ...
    log.debug(
        "qwen_engine.synthesize chunk samples=%d is_final_chunk=%s",
...
class QwenEngine:
    ...
    Unlike ``ChatterboxEngine``, there is no shared mutable
    ``model._conds`` to race on: every ``synthesize()`` call passes its
    own ``ref_audio`` straight through to ``generate()`` as a plain
    argument, so concurrent calls on the same ``model_id`` with
    different reference clips do not interfere with each other.
    """
    ...
    def warm(self) -> None:
        ...
        for _ in self.synthesize(
            ...
            pass
        log.info("qwen_engine.warm done model_id=%s", self._model_id)
    ...
    def synthesize(
        ...
        for result in model.generate(
            ...
            if cancel.is_set():
                log.debug(
                    "qwen_engine.synthesize cancelled model_id=%s chunk_count=%d",
            ...
            yield pcm
        log.debug(
            "qwen_engine.synthesize done model_id=%s chunk_count=%d total_samples=%d",
