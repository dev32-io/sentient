...
plus building the ``Done`` event fields and the ``chatterbox.synthesize``
...
# both the `Done` WS event and the `chatterbox.synthesize` JSONL record.
...
def build_synth_record(
    *, metrics: Metrics, voice_id: str | None, format: str, sample_rate: int,
    ...
    """Full ``chatterbox.synthesize`` JSONL record: the ``Done`` fields plus
