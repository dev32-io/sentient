...
#define OPUS_FRAME_DURATION_MS 20 // Sentient gateway expects 20 ms opus frames (Phase 6 cube-sdk pivot).
...
public:
    ...
    //
    // Thread-safety: codec output calls (EnableOutput, OutputData) follow the
    // same pattern as PlaySound. Callers should not invoke PlayPcm concurrently
    // with PlaySound from multiple tasks.
    ...
    //
    // Thread-safety: the devtool verb task is the only caller. AudioInputTask
