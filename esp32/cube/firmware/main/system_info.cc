...
void SystemInfo::LogHeap(const char* where) {
    if (where == nullptr) where = "?";
    size_t f_int   = heap_caps_get_free_size(MALLOC_CAP_INTERNAL);
    size_t m_int   = heap_caps_get_minimum_free_size(MALLOC_CAP_INTERNAL);
    size_t lb_int  = heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL);
    size_t f_psram = heap_caps_get_free_size(MALLOC_CAP_SPIRAM);
    size_t m_psram = heap_caps_get_minimum_free_size(MALLOC_CAP_SPIRAM);
    ESP_LOGI(TAG, "heap@%s sram free=%u min=%u largest=%u | psram free=%u min=%u",
             where, (unsigned)f_int, (unsigned)m_int, (unsigned)lb_int,
             (unsigned)f_psram, (unsigned)m_psram);
}
