    ...
    if (app.GetDeviceState() == kDeviceStateIdle) {
        if (last_status_update_time_ + std::chrono::seconds(10) < std::chrono::system_clock::now()) {
            ...
            if (tm->tm_year >= 2025 - 1900) {
                ...
                // Demoted W -> D: cube has no RTC, fires every status tick with
                // zero diagnostic value. Each line allocated heap through the
                // vprintf hooks (UDP relay + ring buffer), starving lwIP's pbuf
                // pool mid-chunked-send and stalling /screenshot + /audio/record
                // (T33). Re-enable with esp_log_level_set("Display", DEBUG).
                ESP_LOGD(TAG, "System time is not set, tm_year: %d", tm->tm_year);
            }
        }
    }
