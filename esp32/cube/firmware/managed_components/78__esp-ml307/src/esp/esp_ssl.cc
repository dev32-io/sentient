...
const char* EspSsl::s_cacert_pem_buf_ = nullptr;
size_t EspSsl::s_cacert_pem_bytes_ = 0;

void EspSsl::SetCacert(const char* pem_buf, size_t pem_bytes) {
    s_cacert_pem_buf_ = pem_buf;
    s_cacert_pem_bytes_ = pem_bytes;
    ESP_LOGI(TAG, "Pinned CA cert (%u bytes); subsequent Connect() will skip esp_crt_bundle.",
             (unsigned)pem_bytes);
}
...
bool EspSsl::Connect(const std::string& host, int port) {
    ...
    esp_tls_cfg_t cfg = {};
    if (s_cacert_pem_buf_ != nullptr && s_cacert_pem_bytes_ > 0) {
        cfg.cacert_pem_buf = reinterpret_cast<const unsigned char*>(s_cacert_pem_buf_);
        cfg.cacert_pem_bytes = static_cast<unsigned int>(s_cacert_pem_bytes_);
        // When a CA is pinned, exact-cert match IS the identity proof — the
        // CN/SAN check is redundant and would only cause failures when
        // connecting by LAN IP to a cert whose SAN covers only DNS/localhost
        // (e.g., the dev gateway's self-signed cert with CN=sentient).
        // Prod path falls through to crt_bundle_attach where CN check stays
        // mandatory (public CAs require hostname verification).
