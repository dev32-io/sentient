...
### The shared `McpClient` never recovers a catalog server that restarted under it

Found while verifying 9g. `tools/mcp-client.ts` caches one transport per catalog server and evicts it only when the CONNECT itself rejects — never when `listTools`/`callTool` fails. So when the system orchestrator recreates an addon container *after* the gateway has connected (which it does for `ma-mcp` on every dev boot: the boot warm-up connects, then `apply` recreates), every later call to that server returns `-32600 Session not found` for the lifetime of the process, and only a gateway restart clears it.

Consequences, both real: the gateway's own ReAct loop silently loses that server's tools, and the delegated proxy tier does too — measured live, `ma_*` absent from a `tools/list` that correctly carried HA, fetch and searxng. It also caps the "the proxied surface is re-derived at the moment of use, so a late addon needs no restart" property: re-derivation cannot help a *poisoned* connection, only an absent one.

The fix belongs in `mcp-client.ts` (evict on a failed list/call, then redial once) and was left alone here because that file is outside task 9g's ownership.
