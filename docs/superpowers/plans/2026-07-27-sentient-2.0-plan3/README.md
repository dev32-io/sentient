...
**Erratum — the T8 index sweep (recorded; deliberately not rewritten).** Third of the same family, and the first caused by the shared **index** rather than the shared working tree. `71119ef` ("fix(android): finish the cycleId to turnId sweep in the chat row keys") is titled and attributed entirely to T8's Android unblock, but also carries five of T9's files — `ios/App/Chat/message/{ChatRows,MessageBubble,MessageList}.swift` and `ios/Tests/{ChatRowsTests,CycleErrorRecoveryTests}.swift`, 35 insertions / 35 deletions of the mirror-image `cycleId → turnId` rename that T8 never staged. Cause: T9 had `git add`-ed those files; their commit was rejected by a red `lefthook` `pre-commit` (its `typecheck` step is unglobbed and reads the WORKING TREE, so a sibling's in-flight `gateway/**` edits hold it red for *every* task in the tree); the files stayed sitting in the shared index; and T8's commit driver verified only that the index was non-empty, not that the staged path set equalled the paths it had just `git add`-ed — so an indexwide `git commit` swept them in. This violates R12's staging-discipline clause and the one-logical-change-per-commit rule. Consequence: `git bisect` / `git revert` across `71119ef` is unsafe in both directions — reverting the Android row-key fix also reverts T9's iOS rename, and vice versa — and per-task attribution for that SHA is wrong, which misleads any reviewer who diffs `71119ef` expecting Android-only content. Mitigating facts, all verified: the swept hunks are correct and coherent (the exact iOS mirror of the Android `messageRowKey` `cyc-` → `turn-` change), HEAD is green, and T9 never re-committed them — `git log b777256..HEAD -- ios/App/Chat/message/ChatRows.swift …` returns `71119ef` alone, so the branch carries no duplicate or conflicting commit. History is **not** rewritten, for the reasons recorded in the two errata above: nine sibling commits already sit on top of `71119ef`, the wave orchestrator addresses tasks by SHA, and `git stash` / `reset` / `restore` are forbidden to implementers here. **Remedy: the same squash-merge into `develop`.** Flagged for the plan owner — a merge-time decision, not an implementer's to take.

**Prevention (binding for the remaining waves; supersedes the verify-only half of R12's staging discipline).** "Stage by explicit path, then verify `git diff --cached`" is necessary but NOT sufficient here: the index is shared too, so a sibling can add paths to it before — or between — your `git add` and your `git commit`, and a bare `git commit` commits the whole index regardless of what you verified a moment earlier. **Commit with an explicit pathspec instead: `git commit -m "…" -- <path> [<path>…]`** (equivalently `--only`; it is the default mode whenever paths are given). Git builds a temporary index from just those paths, so foreign staged entries can neither be swept into your commit nor be un-staged out from under their owner — that non-destructive property is what makes this the right tool when `reset` / `restore` / `stash` are forbidden. Three rules come with it: (1) **every option, `-m` included, must precede the `--`** — git treats everything after the separator as a pathspec, so the heredoc-message form is `git commit -m "$(cat <<'EOF' … EOF
)" -- <path>`, and putting `-m` after `--` fails with `error: pathspec '-m' did not match any file(s)`; (2) a pathspec commit takes the **working-tree** content of the named paths, disregarding anything staged for them, so it is equivalent to `git add <paths> && git commit` only when whole files are staged — never use it after a partial `git add -p`; (3) still assert the set before committing (`git status --porcelain -- <paths>`, or `git diff --cached --name-only` for the indexwide form) — "the index is non-empty" is not that check, and it is the check whose absence produced this erratum.
