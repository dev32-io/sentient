...
| Task strip persists until the next turn starts | desktop 1280×900 | the turn above, completing | Wait for `turn.completed`, then send a new message | Row settles to done/error and STAYS after `turn.completed` (composer keeps its expanded height); it clears only once the new turn starts | `task-list.turn-ended` (rows unchanged); `task-list.turn-started rows=0` on the next turn; matching `turn-emitter.task-list` frames |
