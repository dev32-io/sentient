...
2. **Write-time scanning, fail-closed — per-surface severity.** Everything written into file memory — by memory tools *or* the dreamer — passes `scanContent` (skill-tools precedent: `injectionScanError`). For **prompt-rendered** surfaces (`MEMORY.md`, topic files) a `suspicious`/`hostile` verdict refuses the write, because they render into the system prompt with no read-time gate. The **journal** is not prompt-rendered — its content reaches model context exclusively via the deep-memory index → spark/`memory_recall` → the inbound `memory_body` gate (§3.3, a second screen with risk escalation) — so a journal write refuses `hostile` ONLY; a `suspicious` verdict is annotated (WARN, counts/categories only) and written, with taint provenance still marking tool-derived episodes. (This keeps faithful dreamer summaries — which routinely trip the `context_manipulation` pattern with phrasings like "the user confirmed …" — from being deterministically refused.) Invisible-unicode rejection as in `skill-file.ts`. **Direct human file edits are an ingest event**: the store detects out-of-band changes by content hash on session build and on index sync; changed files are re-validated and re-scanned before rendering or indexing — a failed scan quarantines the file (WARN, rendered as absent) rather than injecting it, on the same per-surface severity split (journal quarantines on `hostile` only).
