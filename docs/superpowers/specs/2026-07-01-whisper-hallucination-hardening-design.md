...
```yaml
  ...
  rms_energy_floor: 0.005   # ~-46 dBFS starting point; tune from logs
  # Known Whisper filler-hallucination phrases (normalized: lowercase, punctuation
  # stripped). Seeded from the HF sachaarbonel/whisper-hallucinations blocklist +
  # research "Bag of Hallucinations" — YouTube-subtitle training artifacts Whisper
  # emits on noise. Dropped ONLY when the segment is ALSO short or quiet, so a
  # real, clearly-spoken phrase survives. ("you"/"the"/"so" are borderline real
  # words — kept only because the short+quiet guard protects loud real ones.)
  # Generic conversational phantoms (Yeah/Hmm/All right) are NOT artifacts — the
  # rms_energy_floor catches those, not this list.
