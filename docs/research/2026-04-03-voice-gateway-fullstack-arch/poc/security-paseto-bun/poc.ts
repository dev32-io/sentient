...
async function testPasetoBasic() {
  ...
  {
    // Create token with exp 2s in the future, then wait for expiry
    const iat = new Date().toISOString();
    const exp = new Date(Date.now() + 2000).toISOString();
    const token = await encrypt(key, { sub: "kevin", role: "adult", iat, exp } as any);
...
async function testPasetoLatency() {
  ...
  for (let i = 0; i < keyGenIter; i++) generateKeys("local");
