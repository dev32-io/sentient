...
**CRITICAL — append, don't insert:** Append new tasks to the END of the unchecked list, right BEFORE the trailing `Synthesize: update architecture.md`. Never insert tasks in the middle of existing unchecked items. The current scoring round must complete before expansion work begins. This ensures Score tasks are not pushed back.
