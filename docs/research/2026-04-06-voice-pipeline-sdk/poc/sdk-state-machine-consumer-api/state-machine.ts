// ============================================================
// SDK State Machine — Pure function core + thin runtime wrapper
// This is the "black box" that ships in the SDK.
// ============================================================
...
// --- User-friendly status (what developers actually see) ---

export interface VoiceStatus {
  state: VoiceState;
  label: string;           // "Listening...", "Thinking...", etc.
  canSpeak: boolean;       // true when user input is accepted
  isActive: boolean;       // true when pipeline is doing something
  transcript?: string;     // latest partial/final transcript
...
export function transition(state: VoiceState, event: VoiceEvent): TransitionResult {
  // Global event: SESSION_END from any state
  ...
  switch (state) {
    ...
    case "processing":
      if (event.type === "TRANSCRIPT_PARTIAL" || event.type === "TRANSCRIPT_FINAL") {
        return { state: "processing", effects: [] }; // UI update handled by status
      }
  ...
  // Unhandled event for this state — no transition
  return {
    state,
...
// --- Runtime wrapper (what the developer imports) ---
...
export class VoiceClient {
  ...
  /** Current voice status — always available, even without subscribing */
  ...
  /** Register a handler that executes side effects (WS, audio, etc.) */
  ...
  send(event: VoiceEvent): void {
    ...
    // Track transcript and error for status
    if (event.type === "TRANSCRIPT_PARTIAL" || event.type === "TRANSCRIPT_FINAL") {
  ...
  // --- Convenience methods (sugar over send()) ---
